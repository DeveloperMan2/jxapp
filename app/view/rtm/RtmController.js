...
Ext.define('jxapp.view.rtm.RtmController', {
    ...
    loadPles: function () {
        ...
        function successCallBack(response, opts) {
            var mi = L.icon({
                iconUrl: 'resources/images/jxapp/people.png',
                iconSize: [24, 24],
                iconAnchor: [12, 24]
            });
            var markers = [];
            ...
            if (result) {
                ...
                if (markers.length > 0) {
                    ...
                    conf.map.instance.addLayer(r.plesMarkerGroup);
                }
            }
        }
    ...
    cellclickHandler: function (element, td, cellIndex, record, tr, rowIndex,
        ...
        if (cellIndex == 0 && record.data.type == 'reservoir'
            ...
            conf.map.instance.flyTo(mark, conf.map.instance._layersMaxZoom.length - 2);
        }
    },
    ...
    createReservoir: function (nodes) {
        ...
        if (nodes != null && nodes.length > 0) {
                ...
                if (node && node['children'] && node['children'].length > 0) {
                    ...
                    Ext.Array.each(subNodes, function (data) {
                        if (data && data.type == "reservoir") {
                            // markers.push(L.marker([data.lat, data.lgt], {
                            // 							title : data.task
                            // 						})
            ...
            if (markers.length > 0) {
                ...
                conf.map.instance.addLayer(r.rtmMarkerGroup);
            }
        }
    }
});
